...
- 遊戲進度目前存在記憶體(`GAMES`)，重啟就清空；人多時改用 Redis 或 SQLite
- 部署可用 Render、Railway、Fly.io，或學校主機搭配 `gunicorn app:app`
- 只想放靜態網頁：執行 `python build_standalone.py`，上傳 `standalone.html` 即可（沒有新聞分類，答案在網頁原始碼裡）
