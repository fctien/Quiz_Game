...
### 步驟 3-1　設定 TEACH_CODE（課程題庫的解鎖碼）

**這一步不做，Render 上就看不到課程主題**（Python、Deep Learning、AI 導論、VBA、EMBA）。

課程題目是上課教材，不能讓學生自己把整包撈走。所以在雲端上，這五個主題預設是關著的，
要輸入解鎖碼才會出現。解鎖碼放在 Render 的環境變數裡，**不寫進任何檔案**——repo 是公開的。

1. Render → 你的服務 → 左邊 **Environment** → **Add Environment Variable**
2. Key 填 `TEACH_CODE`，Value 填一組你自己記得住的字串（建議 10 碼以上，別用課程代碼）
3. 存檔後 Render 會自動重新部署，等它變回綠色的 Live

另外可以再加一個 `SECRET_KEY`（隨便一串亂碼）。不設也能用，只是伺服器每次重啟後
（免費方案 15 分鐘沒人用就會休眠）要重新解鎖一次。

**設計上刻意的行為**：如果沒設 `TEACH_CODE`，Render 上的課程主題會整批消失，而且解不開。
寧可你發現「咦課程不見了」，也不要以為鎖好了其實沒鎖。

本機（`啟動-Windows.bat`）不受影響：沒設 `TEACH_CODE` 就是全開，跟以前一模一樣，不用多做設定。
...
3. 看到這樣的內容就成功：
   ...
   ```
4. 再開 `https://<你的網址>/` 確認首頁正常，版本字樣應該是 **v6.2**。
5. 開 `https://<你的網址>/host`，主題上方應該有四個分頁：**課程（需解鎖）／通識／輕鬆／特別**。
   按「課程」→ 輸入 `TEACH_CODE` → 五個課程主題出現，就代表設定成功。
   解鎖記在瀏覽器上，12 小時內不用再輸入。
...
| 畫面還是舊版 | 右上角看版本字樣。不是 v6.2 就是瀏覽器快取，按 Ctrl+Shift+R |
